...
import com.aihoo.domain.drug.mapper.HospitalMapper;
...
import com.aihoo.domain.drug.util.LoginRecordUtil;
 ...
 * 医院 服务实现（t_hospital）。
 *
 * <p>迁自 aihoo-biz-service/aihoo-admin 的 HospitalServiceImpl。
 *
 * <p>不调 doctor 域：原 doctorCount 统计逻辑已去除（按 user 决策）。
 * <p>桩实现：Excel 导入导出相关方法不在 admin-api 当前 controller 端点列表中，本期不迁。
 */
@Slf4j
@Service
    ...
    public JSONObject hospitalUpdate(Map<String, Object> map, HttpServletRequest request) {
        ...
        if (null != map.get("departCodeArray") && !"".equals(map.get("departCodeArray")) && !newIds.isEmpty()) {
            ...
            if (updateIds.isEmpty()) {
                ...
                // 不再校验医生关联（已按 user 决策不调 doctor 域），直接删除 + 新增
